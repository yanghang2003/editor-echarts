...
//一进入这个页面先判断是否把饼图初始数据赋值给store
const store = useStore();
const newBarConfig = JSON.parse(JSON.stringify(DEFAULT_PIE_CHART_DATA));

store.state.editData.chartType && store.state.editData.chartType == 'pie'
  ? console.log('不需要赋初始值')
...
// 监听数据变化
watch(() => chartTitle, debouncedUpdateChart, { deep: true });
